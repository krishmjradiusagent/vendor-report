...
import logoTopNav from '../../assets/logo-topnav.png'

interface TopNavProps {
  /** Slot for host application to inject context (e.g. Environment selector) */
  contextSlot?: React.ReactNode
  /** Slot for host application to inject user profile area */
  userSlot?: React.ReactNode
  /** Slot for host application to inject custom logo */
  logoSlot?: React.ReactNode
}

export const FlowTopNav: React.FC<TopNavProps> = ({ contextSlot, userSlot, logoSlot }) => {
  return (
        ...
        {userSlot || (
          ...
          >
            <div className="size-12 rounded-full border border-[#EFEFEF] p-px overflow-hidden shadow-sm">
              <img src="https://i.pravatar.cc/150?u=vanessa" alt="User" className="size-full rounded-full object-cover" />
            </div>
            <div className="flex flex-col items-start gap-0.5">
              <span className="text-[16px] font-semibold text-[#303030] leading-none">Vanessa Brown</span>
              <div className="flex items-center gap-1.5">
                <div className="size-2 rounded-full bg-[#5A5FF2]" />
                <span className="text-[12px] font-regular text-[#303030]">Radius Agent</span>
              </div>
            </div>
