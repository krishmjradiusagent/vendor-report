...
const columns: ColumnDef<VendorTransaction>[] = [
    ...
    cell: ({ row }) => {
      ...
      return (
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-[12px] font-semibold text-[#374151] truncate max-w-[130px]">{name}</span>
        </div>
      )
    },
  },
  {
    ...
    size: 180,
    cell: ({ getValue }) => (
      <span className="text-[12px] font-medium text-[#374151] block truncate max-w-[150px]">{getValue() as string}</span>
    ),
  },
  {
    ...
    size: 200,
    cell: ({ getValue }) => (
      <span className="text-[12px] font-medium text-[#374151] block truncate max-w-[180px]">{getValue() as string}</span>
    ),
  },
  {
    ...
    size: 180,
    cell: ({ getValue }) => (
      <span className="text-[12px] font-medium text-[#374151] block truncate max-w-[140px]">{getValue() as string}</span>
    ),
  },
  {
    ...
    size: 110,
    cell: ({ getValue }) => <span className="text-[12px] font-medium text-[#374151] whitespace-nowrap">{getValue() as string}</span>,
  },
  {
...
export function VendorTable({ data, globalFilter }: { data: VendorTransaction[], globalFilter: string }) {
  ...
  return (
    <div className="flex flex-col gap-0 h-full overflow-hidden">
      <div className="w-full flex-1 overflow-auto rounded-[20px] border border-[#E5E7EB] bg-white shadow-[0_32px_64px_-12px_rgba(0,0,0,0.06)] custom-scrollbar scroll-smooth">
              ...
              {table.getRowModel().rows.length === 0 ? (
                <motion.tr key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
                  <td colSpan={columns.length} className="py-24 text-center">
                    <p className="text-[13px] font-medium text-[#9CA3AF]">No transactions found.</p>
                  </td>
                </motion.tr>
              ) : (
                table.getRowModel().rows.map((row, i) => (
                  <motion.tr
                    ...
                    transition={{ duration: 0.4, delay: Math.min(i * 0.01, 0.2), ease: BEZIER }}
                    className="bg-white hover:bg-[#FAFBFF] h-[64px] border-b border-[#F3F4F6]"
                  >
                    {row.getVisibleCells().map((cell) => (
                      <td key={cell.id} className="px-5 py-3 align-middle">{flexRender(cell.column.columnDef.cell, cell.getContext())}</td>
                    ))}
                  </motion.tr>
                ))
              )}
              ...
              <td colSpan={columns.length} className="px-6 py-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-[13px] font-bold text-[#111827]">
                    {totalFiltered} <span className="text-[#9CA3AF] font-medium">Transactions identified</span>
                  </span>
                </div>
              </td>
            </tr>
      ...
      {pageCount > 1 && (
        <div className="flex items-center justify-between py-6 px-2">
          <div className="flex flex-col gap-0.5">
            <span className="text-[12px] font-semibold text-[#111827]">Page {pageIndex + 1} of {pageCount}</span>
          </div>
          <div className="flex items-center gap-2">
            <button onClick={() => table.previousPage()} disabled={!table.getCanPreviousPage()} className="p-2 border rounded-xl disabled:opacity-30"><ChevronLeft className="h-4 w-4"/></button>
            <button onClick={() => table.nextPage()} disabled={!table.getCanNextPage()} className="p-2 border rounded-xl disabled:opacity-30"><ChevronRight className="h-4 w-4"/></button>
          </div>
        </div>
      )}
    </div>
